...
const [returnedJobs, setReturnedJobs] = useState(null);
  ...
  const [fieldsOfInterest, setFieldsOfInterest] = useState(
    "working with machines, working with people"
  );
  ...
  const getJobs = async () => {
    ...
    // setReturnedJobs(json.message.content);
    setLoading(false);
  };

  console.log("retrisdofgbnsdg", returnedJobs)
    ...
    <div className="w-full bg-sky-400 py-6 py-12">
      <div class="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 ">
        ...
        <div className="flex flex-row mt-4 md:mt-6">
            {loading ? (<p>loading</p>) : returnedJobs?.map((job) => (<p>jkasflkasf</p>))}
            {/* {returnedJobs ? returnedJobs.map((job) => (<div className="flex flex-row mt-4 md:mt-6 p-1 w-1/3">
            <div class=" p-5 space-y-4 flex flex-col bg-white border border-gray-200 rounded-xl ">
                ...
                <div>
                  ...
                  >
                    ...
                    <span class="relative z-10 justify-center items-center gap-x-1.5 hidden peer-checked:flex peer-checked:text-gray-800 text-gray-200 ">
                      <svg
                        class="shrink-0 size-3.5 mt-0.5"
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
              ...
              <div>
                ...
                <h3 class="text-sm text-gray-500">{job.pay_rate}</h3>
